<script lang="ts">
  ...
  const LOADING_MSGS = [
    ...
    "Sorting the legends from the rest...",
  ...
  function makeParticle(burst: boolean) {
    ...
    return {
      ...
      y: burst
        ...
        : -20 - Math.random() * canvas.height * 0.5, // stagger start heights
      vx: (Math.random() - 0.5) * (burst ? 10 : 2),
      vy: burst ? -11 - Math.random() * 9 : 0.6 + Math.random() * 1.2, // slower rain
      w: 5 + Math.random() * 8,
      h: 3 + Math.random() * 4,
  ...
  function startConfetti() {
    ...
    function draw() {
      ...
      for (let i = pool.length - 1; i >= 0; i--) {
        ...
        p.y   += p.vy;
        p.rot += p.vrot;
        p.vy  += p.burst ? 0.32 : 0.018; // very gentle gravity for rain
...
<!-- Confetti canvas — full screen, fewer slower particles -->
...
<div class="min-h-screen flex items-center justify-center" style="background:{$theme==='twilio'?'#f0f0f2':'#06080f'}">
  <div style="text-align:center;padding:0 24px">
    ...
    <!-- Cycling message -->
    <div style="margin-top:20px;font-size:{$theme==='p5'?'16px':'15px'};font-weight:{$theme==='p5'?'700':'600'};font-style:{$theme==='p5'?'italic':'normal'};color:{$theme==='p5'?'rgba(255,184,0,0.8)':'var(--text-muted)'};letter-spacing:{$theme==='p5'?'0.08em':'0.02em'};animation:fadeMsg 0.4s ease;min-height:24px">
      {loadingMsg}
    </div>
    <!-- Loading bar -->
...
<!-- Ticker — seamless loop via doubled content -->
<div style="position:fixed;bottom:{$theme==='p5'?'4px':'0'};left:0;right:0;height:52px;background:{$theme==='twilio'?'white':'#0d1117'};border-top:{$theme==='twilio'?'1px solid rgba(13,18,43,0.1)':'2px solid rgba(255,184,0,0.2)'};z-index:997;overflow:hidden;display:flex;align-items:center;{$theme==='twilio'?'box-shadow:0 -1px 8px rgba(13,18,43,0.06)':''}">
  <!-- Outer wrapper animates; inner has content doubled for seamless wrap -->
  <div style="display:flex;animation:tickerScroll 32s linear infinite;white-space:nowrap;will-change:transform">
    ...
    {/snippet}
    <!-- Render twice so the seam is invisible -->
...
<div style="max-width:820px;margin:0 auto;padding:80px 20px 80px">
  ...
  <div style="text-align:center;margin-bottom:32px">
    <!-- Twilio logo — both themes -->
    <div style="display:flex;justify-content:center;margin-bottom:18px">
      <img src="/Twilio-logo-red.svg.png" alt="Twilio"
        style="height:28px;width:auto;{$theme==='p5'?'filter:brightness(0) invert(1);opacity:0.5':''}">
    </div>

    {#if $theme === 'twilio'}
      <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.3em;color:var(--act-color);margin-bottom:10px">🏆 Official Results</div>
  ...
  <div style="display:flex;justify-content:center;gap:20px;flex-wrap:wrap;margin-bottom:28px">
    {#each [{val:fmt(data.team_total),lbl:'Team iACV',color:$theme==='twilio'?'#B45309':'#FFB800'},{val:data.team_owl+'%',lbl:'Avg Owl %',color:$theme==='twilio'?'#1D4ED8':'#3B82F6'},{val:String(data.total),lbl:'SEs Ranked',color:$theme==='twilio'?'#178742':'#10B981'}] as s}
  ...
  <div style="display:flex;flex-direction:column;gap:14px">
    ...
    {@const owlCol = se.owl < 75 ? ($theme==='twilio'?'#DC2626':'#EF4444') : se.owl === 100 ? ($theme==='twilio'?'#B45309':'#FFB800') : ($theme==='twilio'?'#6B7280':'#9ca3af')}
      ...
      <div style="border-radius:16px;background:{$theme==='twilio'?'white':'#0c1220'};overflow:hidden;position:relative;border:1px solid {c.color}40;cursor:default;{isOne?`animation:${glowAnim} 2.5s ease infinite`:isDev?'animation:devWiggle 3s ease infinite':''};{$theme==='twilio'?'box-shadow:'+( isHovered ? '0 8px 32px rgba(13,18,43,0.16)' : '0 2px 8px rgba(13,18,43,0.08)'):''}">
        ...
        <div style="position:relative;z-index:1;padding:22px 26px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px">
            <div style="background:{$theme==='twilio'?'var(--dark)':'rgba(255,255,255,0.04)'};border:1px solid {$theme==='twilio'?'rgba(13,18,43,0.08)':'rgba(255,255,255,0.06)'};border-radius:10px;padding:11px 13px">
              <div style="font-size:9px;text-transform:uppercase;letter-spacing:.12em;color:var(--text-muted);margin-bottom:4px">🦉 Owl%</div>
              <div style="font-size:1rem;font-weight:800;color:{owlCol}">{se.owl}%</div>
              <div style="background:{$theme==='twilio'?'rgba(13,18,43,0.08)':'rgba(255,255,255,0.06)'};border-radius:99px;height:3px;overflow:hidden;margin-top:6px">
            ...
            <div style="background:{$theme==='twilio'?'var(--dark)':'rgba(255,255,255,0.04)'};border:1px solid {$theme==='twilio'?'rgba(13,18,43,0.08)':'rgba(255,255,255,0.06)'};border-radius:10px;padding:11px 13px">
              <div style="font-size:9px;text-transform:uppercase;letter-spacing:.12em;color:var(--text-muted);margin-bottom:4px">🔮 Future Q%</div>
              <div style="font-size:1rem;font-weight:800;color:{$theme==='twilio'?'#7C3AED':'#a78bfa'}">{se.future_pct}%</div>
              <div style="background:{$theme==='twilio'?'rgba(13,18,43,0.08)':'rgba(255,255,255,0.06)'};border-radius:99px;height:3px;overflow:hidden;margin-top:6px">
                <div style="height:3px;border-radius:99px;background:{$theme==='twilio'?'#7C3AED':'#8b5cf6'};width:0;animation:barFill .9s cubic-bezier(.34,1.56,.64,1) {delay+800}ms forwards;--w:{se._fw}%"></div>
              </div>
            </div>
            <div style="background:{$theme==='twilio'?'var(--dark)':'rgba(255,255,255,0.04)'};border:1px solid {$theme==='twilio'?'rgba(13,18,43,0.08)':'rgba(255,255,255,0.06)'};border-radius:10px;padding:11px 13px">
              <div style="font-size:9px;text-transform:uppercase;letter-spacing:.12em;color:var(--text-muted);margin-bottom:4px">✉️ Fut. Emails</div>
              <div style="font-size:1rem;font-weight:800;color:{$theme==='twilio'?'#BE185D':'#f9a8d4'}">{se.future}</div>
            </div>
          </div>
        </div>
      </div>
