...
export async function getSFMismatch(team: string, period: string, subteam = '') {
  const r = await fetch(`/api/se-scorecard-v2/data/mismatch?team=${team}&period=${period}${subteamParam(subteam)}`);
  if (!r.ok) return null;
  return r.json();
}
